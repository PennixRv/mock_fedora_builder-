...
if [ "$(docker images | awk '{ print $1 }' | grep "$stage2")" = "" ];then
    ...
    docker exec $stage1_container bash -c \
        ...
        mock -r ./$MOCK_CONFIG --rootdir /home/riscv/mock_root_dir --resultdir /home/riscv/mock_result_dir \
            --install sudo kmod anaconda lorax git vim pykickstart dnf hfsplus-tools lorax-lmc-novirt wget \
...
if [ "$BUILDING_TARGET" = "1" ]; then
    ...
    if [ "$TARGET_IMAGE_TYPE" = "1" ]; then
        ...
        if [ ! -f "visionfive2_fw_payload.img" ];then \
            wget  --progress=dot https://github.com/starfive-tech/VisionFive2/releases/download/JH7110_VF2_515_v3.9.3/visionfive2_fw_payload.img; fi && \
        ...
        for i in {0..7}; do \
            if [ ! -e /dev/loop$i ]; then sudo mknod -m660 /dev/loop$i b 7 $i; fi \
        done && \
